...
* Program Name : grid.js
* Location     : Common Grid
...
function gf_pageIndexHtml(pageInfo, pageFunction, gridPageInfo, gridPageSummary){
    ...
    // [이전 10개] 처리
    if(currPageNo > 10){
        pageInfoHtml += '<a href="#" onClick="gf_searcgPageList(' + preBlockPage + ',' + pageFunction + ');" class="btn btn_forward">이전 10 페이지 보기</a>';
    } else {
    ...
    // 이전페이지
    if(currPageNo > 1){
        pageInfoHtml += '<a href="#" onClick="gf_searcgPageList(' + (Number(currPageNo) - 1) + ',' + pageFunction + ');" class="btn btn_prev">이전 페이지</a>';
    } else {
    ...
    for(var i=strtNo; i<endNo; i++){
        if(i == currPageNo){
            ...
            pageInfoHtml += '<li><a href="#" onClick="gf_searcgPageList(' + i + ',' + pageFunction + ');">' + i + '</a></li>';
        }
    ...
    if(currPageNo < totalPage){
        pageInfoHtml += '<a href="#" onClick="gf_searcgPageList(' + (Number(currPageNo) + 1) + ',' + pageFunction + ');" class="btn btn_next">다음 페이지</a>';
    } else {
    ...
    if(endBlock < totalBlockCount){
        pageInfoHtml += '<a href="#" onClick="gf_searcgPageList(' + nextBlockPage + ',' + pageFunction + ');" class="btn btn_backward">다음 10페이지 보기</a>';
    } else {
 ...
 *******************************************************/
function gf_getRowCnt(listAreaId){
 ...
 * @description 행 풀어주기
 *******************************************************/
function gf_releaseGridRow(targetId){
...
function gf_insertRow(){
    ...
    if(targetTrObj.length == 1 && $("#" + targetId + " #noResultList").length == 1){
        targetTrObj.remove();
    }
...
function gf_insertHtml(){
    ...
    if(locOpt == "before"){               // 선택한 행 before
        targetObj.before(addHtml);
    } else if(locOpt == "after"){         // 선택한 행 after
        targetObj.after(addHtml);
    } else if(locOpt == "first"){         // first
        ...
        if($("#" + targetId + " input[id=" + checkId + "]:eq(" + rowIdx + ")").is(":checked")){
            if(deleteOpt == "remove"){
                ...
                if(crudFlagObj.val() == "R"){
                    crudFlagObj.val("D");    // 실제 삭제
                } else if(crudFlagObj.val() == "C"){
                    crudFlagObj.val("X");    // 실제 삭제 안함
                }
