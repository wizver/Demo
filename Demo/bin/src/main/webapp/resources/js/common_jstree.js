 ...
 *******************************************************/
function gf_setPopupMask() {    // 마스킹
    var mask_layer = $('<div id="mask_layer"></div>');
    
    mask_layer.appendTo("#content");
    
    return false;
}

function gf_setPopupFrame(pop_w, pop_h) {    // 팝업 기본 구조 세팅
    var pop_frame = $("<div id=\"pop_frame\"></div>").appendTo("#mask_layer");
    
    pop_frame.css({ "width":pop_w, "height":pop_h });
}
...
function gf_openPop(popUrl, wid, hei, obj) {
    ...
    gf_setPopupMask();
    gf_setPopupFrame(pop_w, pop_h);
    
    var pop_frame = $("#pop_frame");
    
    pop_frame.load(popUrl, function (){
        // Calendar
        gf_initialCalendar("pop_frame");
        
        var contentWidth     = $("#content").width();        // body content width !!
        var popupWidth       = $("#pop_frame").width();      // popup width !!
        var contentScrollTop = $("#content").scrollTop();    // body content scrollTop !!
        
        var popupTop  = contentScrollTop - 100;
        var popupLeft = (contentWidth / 2) - (popupWidth / 2);    // popup left !!
        
        //alert(contentWidth + " / " + popupWidth + " / " + popupTop + " / " + popupLeft);
        
        // Popup Default CSS Setting !!
        pop_frame.css({ "position":"absolute"
                       ,"width"   : (pop_w     + "px")
                       ,"left"    : (popupLeft + "px")
                       ,"top"     : (popupTop  + "px")
                     });
...
function gf_initialCalendar(){
    ...
    $("#" + targetId + " .date_picker").each(function () {
        ...
        if(opt == "dynamic"){
            $("#" + targetId).find("input[id=" + id + "]").removeClass("hasDatepicker");
        }
 ...
 * 
 * @param {object} data
 * @param {string} opt : checkbox, default
 * @returns N/A
 * @description 트리뷰 만들기
 *******************************************************/
function gf_commMakeTreeView(){
    var args = gf_commMakeTreeView.arguments;
    
    var data = args[0];
    var opt  = args.length > 1 ? args[1] != "" ? args[1] : "default" : "default";
    
    // treeView Delete !!
    $("#jstreeView").jstree("destroy");
    
    // TreeView Make !!
    gf_makeTreeViewParentNode(data, opt);
}

/*===========================================================
 * 리스트 부모 표출 !!
 *===========================================================*/
function gf_makeTreeViewParentNode(data, opt){
    var bodyHtml = "";
    
    bodyHtml += '    <ul>';
    bodyHtml += '        <li data-jstree={"opened":true}>';
    bodyHtml += '            <span>' + data.corpNm + '</span>';
    bodyHtml += '            <ul>';
    ...
    for(var i=0; i<listObj.length; i++){
        ...
        var param = "'" + id + "','" + upprId + "','" + nm + "','" + lvl + "','" + pathId + "','" + pathNm + "','" + isLeaf + "'";
        
        if(Number(lvl) == 1){
            bodyHtml += '<li name="lvl1" data-jstree={"opened":true}>';
            
            if(opt == "checkbox"){
            	bodyHtml += '    <a href="#" id="' + id + '" rel="' + pathNm + '">' + nm + '</a>';
            } else {
                bodyHtml += '    <a href="#" id="' + id + '" OnClick="gf_treeViewClickEvent(' + param + ');">' + nm + '</a>';
            }
    ...
    $("#jstreeView").html(bodyHtml);
    
    gf_makeTreeViewChildNode(data, opt);
}

/*===========================================================
 * 리스트 자식 표출 !!
 *===========================================================*/
function gf_makeTreeViewChildNode(data, opt){
    var k = 1;
    
    var listObj = data.listInfo;
    
    while( $("li[name=lvl" + k + "]").length > 0 ){
        $("li[name=lvl" + k + "]").each(function() {
            var rid = $(this).children().attr("id");
            ...
            for(var i=0; i<listObj.length; i++){
                ...
                if(upprId == rid && Number(lvl) == (k + 1)){
                    bodyHtml += '<li name="lvl' + (k + 1) + '">';
 ...
 * Make TreeView !!
 *===========================================================*/
function gf_makeTreeView(){
    $("#jstreeView").jstree()
   .bind("dblclick.jstree", function (e, data) {    // node가 더블클릭 될때 처리
 ...
 * Make TreeView !!
 *===========================================================*/
function gf_makeChkTreeView(){
    $("#jstreeView").jstree({ "checkbox" : { "keep_selected_style" : false }
                             ,"plugins" : [ "checkbox" ]
                           });
    
    //$('#jstreeView').jstree('open_node', '#HCD');
}